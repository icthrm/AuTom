...
IplImage* drawFloatGraphWithXYLabel(const float *arraySrc, int nArrayLength, const char * xlabel, const char * ylabel, IplImage *imageDst, float minV, float maxV, int width, int height, char *graphLabel, bool showScale){
  printf("drawFloatGraph\n");
	int w = width;
	int h = height;
	int b = 10;		// border around graph within the image
	...
	// Draw the horizontal & vertical axis
	int y0 = cvRound(minV*fscale);
	cvLine(imageGraph, cvPoint(b,h-(b-y0)), cvPoint(w-b, h-(b-y0)), BLACK);//x line
	cvLine(imageGraph, cvPoint(w-b, h-(b-y0)), cvPoint(w-b-4, h-(b-y0)-4), BLACK);
	cvLine(imageGraph, cvPoint(w-b, h-(b-y0)), cvPoint(w-b-4, h-(b-y0)+4), BLACK);
	cvLine(imageGraph, cvPoint(b,h-(b)), cvPoint(b, h-(b+s)), BLACK);//y line
	cvLine(imageGraph, cvPoint(b, h-(b+s)), cvPoint(b-4, h-(b+s)+4), BLACK);
	cvLine(imageGraph, cvPoint(b, h-(b+s)), cvPoint(b+4, h-(b+s)+4), BLACK);
	// Write the scale of the y axis
	CvFont font;
	cvInitFont(&font,CV_FONT_HERSHEY_PLAIN,0.55,0.7, 0,1,CV_AA);	// For OpenCV 1.1
	if (showScale) {
		...
		CvScalar clr = GREY;
		char text[16];
		sprintf(text, "(%d,%.1f)", loc, maxV);
		int y = cvRound((arraySrc[loc] - minV) * fscale);	// Get the values at a bigger scale
		int x = cvRound(loc * xscale);
		cvPutText(imageGraph, text, cvPoint(b+x, h-(b+y)), &font, clr);
		// Write the scale of the x axis
		sprintf(text, "%d", (nArrayLength-1) );
		cvPutText(imageGraph, text, cvPoint(w-b+4-5*strlen(text), (h/2)+10), &font, clr);
	}
	...
	cvInitFont(&font,CV_FONT_HERSHEY_TRIPLEX,0.45,0.4, 0,1);
	if(NULL != xlabel && strlen(xlabel) > 0){
	  cvPutText(imageGraph, xlabel, cvPoint(w-b-7*strlen(xlabel), h-(b-y0)-6), &font, CV_RGB(0, 0, 0));
	}
	
	if(NULL != ylabel && strlen(ylabel) > 0){
	  cvPutText(imageGraph, ylabel, cvPoint(b, h-(b+s)), &font, CV_RGB(0, 0, 0));
	}
...
void showFloatGraphWithXYLabel(const char *name, const float *arraySrc, int nArrayLength, const char * xlabel, const char * ylabel, int delay_ms, IplImage *background){
  #ifdef USE_HIGHGUI
	// Draw the graph
	IplImage *imageGraph = drawFloatGraphWithXYLabel(arraySrc, nArrayLength, xlabel, ylabel, background);
	// Display the graph into a window
    cvNamedWindow( name );
    cvShowImage( name, imageGraph );
