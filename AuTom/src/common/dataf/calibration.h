...
/** used by atomalign */
inline void PrintCalibrationAsImod(const std::vector<calibrated>& calib_v, const util::globalcalib& gcalib, const float rotation, 
                                   const float scale, const char* xffile, const char* anglefile, const char* exfile) {
    std::ofstream out(xffile);
    std::ofstream agout(anglefile);
    std::ofstream exceptf(exfile);
	std::ofstream xtilt("xtilt.params");
	std::ofstream xshift("xaxisshift.params");
    for(int i = 0; i < calib_v.size(); i++) {
        ...
        double dx = calib_v[i].x_shift;//A[0]*calib_v[i].x_shift+A[1]*calib_v[i].y_shift;
        double dy = calib_v[i].y_shift;//A[2]*calib_v[i].x_shift+A[3]*calib_v[i].y_shift;
        ...
        if(calib_v[i].valid) {
			...
			xtilt<<calib_v[i].xtilt/M_PI*180<<std::endl;
        }
        else {
    ...
    xshift<<gcalib.x_axis_shift/M_PI*180<<std::endl;
