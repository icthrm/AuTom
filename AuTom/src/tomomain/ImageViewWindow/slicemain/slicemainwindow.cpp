...
SliceMainWindow::SliceMainWindow(QWidget *parent) :QMainWindow(parent)    ,m_toolBar(nullptr)
    //,m_parser(nullptr)
{
...
SliceMainWindow* SliceMainWindow::ViewMrc(QString filepath)
{
        qDebug()<<"ViewMrc";
        SliceMainWindow *win = new SliceMainWindow();
...
void SliceMainWindow::closeEvent(QCloseEvent* event)
{
...
void SliceMainWindow::initializeForSliceViewWindow()
{
...
void SliceMainWindow::initializeForVolumeViewWindow() {
...
void SliceMainWindow::openMark(const QString & fileName)
{
    ...
    if (fileName.endsWith(QStringLiteral(".mar")) == true)
    {
        ...
        else if (model->dirty() == true)
        {
            ...
            {
                if (SliceMainWindow::saveMark() == false)
                {
...
bool SliceMainWindow::eventFilter(QObject* watched, QEvent* event) {
...
void SliceMainWindow::writeSettingsForDockWidget(QDockWidget * dock, QSettings* settings)
{
    if (settings == nullptr)
    {
...
void SliceMainWindow::readSettingsForDockWidget(QDockWidget* dock, QSettings* settings)
{
    if (settings == nullptr)
    {
        QScopedPointer<QSettings> ptr(new QSettings);
...
void SliceMainWindow::writeSettingsForImageView(SliceEditorWidget * view, QSettings * settings)
{
    if (settings == nullptr)
    {
        QScopedPointer<QSettings> ptr(new QSettings);
...
void SliceMainWindow::readSettingsForImageView(SliceEditorWidget * view, QSettings * settings)
{
    if (settings == nullptr)
    {
        QScopedPointer<QSettings> ptr(new QSettings);
...
void SliceMainWindow::readSettings()
{
    QSettings settings;
    //
...
void SliceMainWindow::writeSettings()
{
    QSettings settings;
    settings.beginGroup(this->windowTitle());
...
void SliceMainWindow::setDefaultLayout()
{
...
void SliceMainWindow::setParallelLayout() {
...
void SliceMainWindow::updateActionsAndControlPanelByWidgetFocus(FocusState state) {
...
void SliceMainWindow::updateActionsBySelectionInSliceView(){
    Q_ASSERT_X(m_imageView, "SliceMainWindow::updateActionsBySelectionInSliceView", "null pointer");
...
void SliceMainWindow::sliceViewSelected(SliceType type) {
...
void SliceMainWindow::initializeForSliceWithMarkingViewWindow() {

}

void SliceMainWindow::createWidget()
{
    ...
    connect(m_imageView, &SliceEditorWidget::viewFocus, this, &SliceMainWindow::sliceViewSelected);
    connect(m_imageView->topView(), &SliceWidget::selectionChanged, this, &SliceMainWindow::updateActionsBySelectionInSliceView);
    ...
    Q_ASSERT_X(m_toolBar, "SliceMainWindow::createWidget", "m_toolBar null pointer");
...
void SliceMainWindow::createMenu()
{
    //File menu
    m_fileMenu = menuBar()->addMenu(QStringLiteral("File"));
    m_fileMenu->addAction(m_openAction);
    m_fileMenu->addAction(m_saveAction);
    m_fileMenu->addAction(QStringLiteral("Close"),this,&SliceMainWindow::close);

    //View menu
    m_viewMenu = menuBar()->addMenu(QStringLiteral("View"));
}

void SliceMainWindow::createActions()
{
    // open action
    ...
    connect(m_saveAction, &QAction::triggered, this, &SliceMainWindow::saveMark);
    ...
    connect(m_setDefaultLayoutAction, &QAction::triggered, this, &SliceMainWindow::setDefaultLayout);
    ...
    connect(m_parallelLayoutAction, &QAction::triggered, this, &SliceMainWindow::setParallelLayout);
}

void SliceMainWindow::createStatusBar()
{
...
void SliceMainWindow::createSliceEditorPlugins() {

}


AbstractSliceDataModel * SliceMainWindow::replaceSliceModel(AbstractSliceDataModel * model)
{
...
MarkModel * SliceMainWindow::replaceMarkModel(MarkModel * model)
{
    return m_imageView->takeMarkModel(model, nullptr);

}

QAbstractTableModel * SliceMainWindow::replaceProfileModel(QAbstractTableModel * model)
{
    return nullptr;
}

QAbstractTableModel * SliceMainWindow::setupProfileModel(const MRC & mrc)
{
