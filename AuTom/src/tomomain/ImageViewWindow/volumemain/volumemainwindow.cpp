...
VolumeMainWindow::VolumeMainWindow(QWidget *parent) :
    QMainWindow(parent)
    ,m_toolBar(nullptr)
    //,m_parser(nullptr)
{
...
VolumeMainWindow* VolumeMainWindow::ViewMrc(QString filepath)
{
        qDebug()<<"ViewMrc";
        VolumeMainWindow *win = new VolumeMainWindow();
...
//          timer->start();
       ...
       win->open(filepath);
        win->show();
        win->setWindowTitle(filepath);

}
void VolumeMainWindow::closeEvent(QCloseEvent* event)
{
...
void VolumeMainWindow::initializeForSliceViewWindow()
{
...
void VolumeMainWindow::initializeForVolumeViewWindow() {
...
bool VolumeMainWindow::saveMark()
{
...
void VolumeMainWindow::openMark(const QString & fileName)
{
    ...
    if (fileName.endsWith(QStringLiteral(".mar")) == true)
    {
        ...
        else if (model->dirty() == true)
        {
            ...
            {
                if (VolumeMainWindow::saveMark() == false)
                {
...
bool VolumeMainWindow::eventFilter(QObject* watched, QEvent* event) {
...
void VolumeMainWindow::writeSettingsForDockWidget(QDockWidget * dock, QSettings* settings)
{
    if (settings == nullptr)
    {
...
void VolumeMainWindow::readSettingsForDockWidget(QDockWidget* dock, QSettings* settings)
{
    if (settings == nullptr)
    {
        QScopedPointer<QSettings> ptr(new QSettings);
...
void VolumeMainWindow::writeSettingsForImageView(SliceEditorWidget * view, QSettings * settings)
{
    if (settings == nullptr)
    {
        QScopedPointer<QSettings> ptr(new QSettings);
...
void VolumeMainWindow::readSettingsForImageView(SliceEditorWidget * view, QSettings * settings)
{
    if (settings == nullptr)
    {
        QScopedPointer<QSettings> ptr(new QSettings);
...
void VolumeMainWindow::readSettings()
{
    QSettings settings;
    //
...
void VolumeMainWindow::writeSettings()
{
    QSettings settings;
    settings.beginGroup(this->windowTitle());
...
void VolumeMainWindow::setDefaultLayout()
{
...
void VolumeMainWindow::setParallelLayout() {
...
void VolumeMainWindow::pixelViewActionTriggered()
{
...
void VolumeMainWindow::histogramViewActionTriggered() {
...
void VolumeMainWindow::updateActionsAndControlPanelByWidgetFocus(FocusState state) {
...
void VolumeMainWindow::updateActionsBySelectionInSliceView(){
    Q_ASSERT_X(m_imageView, "VolumeMainWindow::updateActionsBySelectionInSliceView", "null pointer");
...
void VolumeMainWindow::sliceViewSelected(SliceType type) {
...
void VolumeMainWindow::initializeForSliceWithMarkingViewWindow() {

}

void VolumeMainWindow::createWidget()
{
    ...
    connect(m_imageView, &SliceEditorWidget::viewFocus, this, &VolumeMainWindow::sliceViewSelected);
    connect(m_imageView->topView(), &SliceWidget::selectionChanged, this, &VolumeMainWindow::updateActionsBySelectionInSliceView);
    ...
    Q_ASSERT_X(m_toolBar, "VolumeMainWindow::createWidget", "m_toolBar null pointer");
    ...
    m_resetAction->setIcon(QIcon(":icons/reset.png"));
    ...
    m_markAction->setIcon(QIcon(":icons/mark.png"));
    m_markAction->setCheckable(true);
    ...
    m_markSelectionAction->setIcon(QIcon(":icons/select.png"));
    ...
    m_anchorAction->setIcon(QIcon(":icons/anchor.png"));
    ...
    m_markDeletionAction->setIcon(QIcon(":icons/delete.png"));
    ...
    m_pixelViewAction->setIcon(QIcon(":icons/picker.png"));
    connect(m_pixelViewAction, &QToolButton::clicked, this, &VolumeMainWindow::pixelViewActionTriggered);
    ...
    m_histogramAction->setIcon(QIcon(":icons/histogram.png"));
    connect(m_histogramAction, &QToolButton::clicked, this, &VolumeMainWindow::histogramViewActionTriggered);
    m_toolBar->addWidget(m_histogramAction);

}

void VolumeMainWindow::createMenu()
{
    //File menu
    m_fileMenu = menuBar()->addMenu(QStringLiteral("File"));
    m_fileMenu->addAction(m_openAction);
    m_fileMenu->addAction(m_saveAction);
    m_fileMenu->addAction(QStringLiteral("Close"),this,&VolumeMainWindow::close);

    //View menu
    m_viewMenu = menuBar()->addMenu(QStringLiteral("View"));
}

void VolumeMainWindow::createActions()
{
    // open action
    m_openAction = new QAction(QIcon(":/icons/open.png"), QStringLiteral("Open"), this);
    ...
    //save mark action
    m_saveAction = new QAction(QIcon(":/icons/save_as.png"), QStringLiteral("Save Mark"), this);
    m_saveAction->setToolTip(QStringLiteral("Save Mark"));
    m_toolBar->addAction(m_saveAction);
    connect(m_saveAction, &QAction::triggered, this, &VolumeMainWindow::saveMark);

    //open mark action
    m_openMarkAction = new QAction(QIcon(":/icons/open_mark.png"), QStringLiteral("Open Mark"), this);
    ...
    //set default action
    m_setDefaultLayoutAction = new QAction(QIcon(":/icons/tab_layout.png"),QStringLiteral("Default Layout"),this);
    m_setDefaultLayoutAction->setToolTip(QStringLiteral("Default Layout"));
    m_toolBar->addAction(m_setDefaultLayoutAction);
    connect(m_setDefaultLayoutAction, &QAction::triggered, this, &VolumeMainWindow::setDefaultLayout);

    // Set Parallel Layout Action
    m_parallelLayoutAction = new QAction(QIcon(":/icons/parallel_layout.png"), QStringLiteral("Parallel Layout"), this);
    m_parallelLayoutAction->setToolTip(QStringLiteral("Parallel Layout"));
    m_toolBar->addAction(m_parallelLayoutAction);
    connect(m_parallelLayoutAction, &QAction::triggered, this, &VolumeMainWindow::setParallelLayout);
}

void VolumeMainWindow::createStatusBar()
{
...
void VolumeMainWindow::createSliceEditorPlugins() {

}


AbstractSliceDataModel * VolumeMainWindow::replaceSliceModel(AbstractSliceDataModel * model)
{
...
MarkModel * VolumeMainWindow::replaceMarkModel(MarkModel * model)
{
    return m_imageView->takeMarkModel(model, nullptr);

}

QAbstractTableModel * VolumeMainWindow::replaceProfileModel(QAbstractTableModel * model)
{
    return nullptr;
}

QAbstractTableModel * VolumeMainWindow::setupProfileModel(const MRC & mrc)
{
