...
int main(int argc, char **argv)
{
    ...
    util::GetImgAvgSdvCutoffMinMax(mid, &avg, &dev, &min, &max);
        ...
        util::GetImgAvgSdvCutoffMinMax(img, &avg, &dev, &min, &max);
                ...
                if(m==2){
                        cvWarpAffine(img, newimg, xfmatrix, CV_INTER_LANCZOS4  | CV_WARP_FILL_OUTLIERS, cvScalarAll(avg));
                }
                else{
                        cvWarpAffine(img, newimg, xfmatrix, CV_INTER_CUBIC  | CV_WARP_FILL_OUTLIERS, cvScalarAll(avg));
                }
