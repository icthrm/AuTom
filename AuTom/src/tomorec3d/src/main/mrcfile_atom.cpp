...
int mrc_update_head(MPI_File fout)
{
 ...
 // fclose(finout);
  ...
  PRINT_DEBUG_MSG("head->amin is %f, head->amax is %f, head->amean is %f\n",head->amin, head->amax, head->amean);
 // mrc_replace_head(inoutf,head);

  mrc_write_head(fout, head);
  free(head);
  PRINT_DEBUG_MSG("updating finished!\n");
  return 0;
}
...
//read the rest slices
  ...
  PRINT_DEBUG_MSG("amin is %f, amax is %f, amean is %Lf in process %d\n",amin, amax, amean,id);
...
int mrc_read_slice(MPI_File fin, MrcHeader  *head, int slcN, char axis,float *mrc_data_slice)
{
        ...
        MPI_Offset filesize, headsize, offset;
	...
	switch(head->mode)
	{
	    ...
	    MPI_File_seek( fin, headsize, MPI_SEEK_SET ); 
...
int mrc_read_slice_y(MPI_File fin, MrcHeader *head, int slcN, char axis,
		float *mrc_data_slice) {

	MPI_Offset filesize, headsize;
	...
	case MRC_MODE_BYTE:
		headsize = filesize
				- (long long) head->nx * head->ny * head->nz * sizeof(char);
	...
	case MRC_MODE_USHORT:
		headsize = filesize
				- (long long) head->nx * head->ny * head->nz * sizeof(short);

		break;

	case MRC_MODE_FLOAT:
		headsize = filesize
				- (long long) head->nx * head->ny * head->nz * sizeof(float);
...
//      read y slice
	...
	switch (head->mode) {
	case MRC_MODE_BYTE:
		if ((data_byte = (unsigned char *) malloc(
				sizeof(unsigned char) * Zslice_length)) == NULL) {
			printf(
					"Error with Function 'mrc_read_slice() in axis-Y'!Can't malloc memery for 'data_byte'!");
			return FALSE;
		}
		memset(data_byte, 0, sizeof(unsigned char) * Zslice_length);
		...
		for (k = 0; k < head->nz; k++) {
			MPI_File_seek(fin,
					headsize
							+ (slcN * head->nx + k * Zslice_length)
									* sizeof(unsigned char), MPI_SEEK_SET);
			MPI_File_read(fin, data_byte, head->nx, MPI_UNSIGNED_CHAR,
					MPI_STATUS_IGNORE );

			for (int i = 0; i < head->nx; i++) {
				mrc_data_slice[mrcoffset + i] = (float) data_byte[i];
			}
			mrcoffset += head->nx;
		}
		free(data_byte);
	...
	case MRC_MODE_SHORT:
		mrcoffset = 0;
		if ((data_short = (short *) malloc(sizeof(short) * Zslice_length))
				== NULL) {
			printf(
					"Error with Function 'mrc_read_slice() in axis-Y'!Can't malloc memery for 'data_short'!");
			return FALSE;
		}
		memset(data_short, 0, sizeof(short) * Zslice_length);
		...
		for (k = 0; k < head->nz; k++) {
			MPI_File_seek(fin,
					headsize
							+ (slcN * head->nx + k * Zslice_length)
									* sizeof(short), MPI_SEEK_SET);
			MPI_File_read(fin, data_short, head->nx, MPI_SHORT,
					MPI_STATUS_IGNORE );

			for (int i = 0; i < head->nx; i++) {
				mrc_data_slice[mrcoffset + i] = (float) data_short[i];
			}
			mrcoffset += head->nx;
	...
	case MRC_MODE_USHORT:
		if ((data_unshort = (unsigned short *) malloc(
				sizeof(unsigned short) * Zslice_length)) == NULL) {
			printf(
					"Error with Function 'mrc_read_slice() in axis-Y'!Can't malloc memery for 'data_unshort'!");
			return FALSE;
		}
		memset(data_unshort, 0, sizeof(unsigned short) * Zslice_length);
		...
		for (k = 0; k < head->nz; k++) {
			...
			MPI_File_seek(fin,
					headsize
							+ (slcN * head->nx + k * Zslice_length)
									* sizeof(unsigned short), MPI_SEEK_SET);
			MPI_File_read(fin, data_unshort, head->nx, MPI_UNSIGNED_SHORT,
					MPI_STATUS_IGNORE );

			for (int i = 0; i < head->nx; i++) {
				mrc_data_slice[mrcoffset + i] = (float) data_unshort[i];
			}
			mrcoffset += head->nx;
		}
		free(data_unshort);
	...
	case MRC_MODE_FLOAT:
		if ((data_float = (float *) malloc(sizeof(float) * Zslice_length))
				== NULL) {
			printf(
					"Error with Function 'mrc_read_slice() in axis-Y'!Can't malloc memery for 'data_float'!");
			return FALSE;
		}
		memset(data_float, 0, sizeof(float) * Zslice_length);
		...
		for (k = 0; k < head->nz; k++) {
			MPI_File_seek(fin,
					headsize
							+ (slcN * head->nx + k * Zslice_length)
									* sizeof(float), MPI_SEEK_SET);
			MPI_File_read(fin, data_float, head->nx, MPI_FLOAT,
					MPI_STATUS_IGNORE );
			//memcpy(mrc_data_slice + k * head->nx, data_float,head->nx * sizeof(float));
			for(int i=0;i<head->nx;i++)
				mrc_data_slice[mrcoffset + i] = (float) data_float[i];
			mrcoffset += head->nx;
		}
		free(data_float);
...
int mrc_read_slice_floatseek(MPI_File fin, MrcHeader *head, int slcN, char axis,
		float *mrc_data_slice) {
	MPI_Offset filesize, headsize;
	...
	if ((data_float = (float *) malloc(sizeof(float) * head->nx)) == NULL) {
		printf(
				"Error with Function 'mrc_read_slice() in axis-Y'!Can't malloc memery for 'data_float'!");
		return FALSE;
	}
	memset(data_float, 0, sizeof(float) * head->nx);

	for (k = 0; k < head->nz; k++) {
		MPI_File_seek(fin, headsize + (slcN * head->nx + k * Zslice_length)*sizeof(float),MPI_SEEK_SET);
		MPI_File_read(fin, data_float, head->nx, MPI_FLOAT, MPI_STATUS_IGNORE );
		memcpy(mrc_data_slice + k * head->nx, data_float, head->nx*sizeof(float));
	}
...
int mrc_read_slice_short(MPI_File fin, MrcHeader *head, int slcN, char axis,
		short *mrc_data_slice) {
	MPI_Offset filesize, headsize;
	...
	if ((data_float = (short *) malloc(sizeof(short) * head->nx)) == NULL) {
		printf(
				"Error with Function 'mrc_read_slice() in axis-Y'!Can't malloc memery for 'data_float'!");
		return FALSE;
	}
	memset(data_float, 0, sizeof(short) * head->nx);

	for (k = 0; k < head->nz; k++) {
		MPI_File_seek(fin, headsize + (slcN * head->nx + k * Zslice_length)*sizeof(short),MPI_SEEK_SET);
		MPI_File_read(fin, data_float, head->nx, MPI_SHORT, MPI_STATUS_IGNORE );
		memcpy(mrc_data_slice + k * head->nx, data_float, head->nx*sizeof(short));
	}
...

{
    ...
    
    {
	  ...
	  case MRC_MODE_USHORT:
		  ...
		  data_unshort[x+y*head->nx]=(unsigned short)pixel_gray[0];
		...
		switch(head->mode)
		  {
			 ...
			 case MRC_MODE_FLOAT:
				 ...
				 for(k=0;k<head->nz;k++)
				 {
					 ...
					 for(j=start;j<end;j++)
						 for(i=0;i<head->nx;i++)
							 mrc_data_block[i+(j-start)*head->nx+(long long)k*length*head->nx]=data_float[i+j*head->nx];
				 }
				 free(data_float);
...
int mrc_write_block(MPI_File fout, MrcHeader  *head, int start, int end, char axis,float *mrc_data_block)
{
        ...
        long long  i,k,j,psize;

		psize=sizeof(float);

       // int length;
		long long del=end-start;
...
//check the mrc file to make sure the size is exact in register with the head
	...
	{
		case MRC_MODE_BYTE:
			if(get_file_size(fin) - 1024 != head->nx*head->ny*head->nz*sizeof(char))
			{
		...
		case MRC_MODE_SHORT:
		case MRC_MODE_USHORT:
			if(get_file_size(fin) - 1024 != head->nx*head->ny*head->nz*sizeof(short))
			{
				printf("Error with Function 'mrc_read_all()'! File size erro!");
			}
			break;
		...
		case MRC_MODE_FLOAT:
			if(get_file_size(fin) - 1024 != head->nx*head->ny*head->nz*sizeof(float))
			{
				printf("Error with Function 'mrc_read_all()'! File size erro!");
			}
			break;
