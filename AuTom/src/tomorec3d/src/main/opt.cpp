...
void usageDual() {
	...
	printf("--mode/-m BPT/\n");
	printf("  Using the direct back projection reconstructed method BPT\n");
	
	printf("--mode/-m FBP/-F filter name\n");
	printf("  Name of the filter for the reconstructed method FBP\n");

	printf("--mode/-m SART Iteration numbers, relax parameter\n");
	printf(
			"  Iteration numbers and relax parameter for the reconstructed method SART\n");

	printf("--mode/-m SIRT Itertaion numbers, relax parameter\n");
	printf(
...
void usageSig() {
	...
	printf("Usage: tomorec3d [options] [input_file] [output_file]\n");
	...
	printf("--rotate/-r 0/1");
	printf("  rotate the reconstruction result\n");
...
int GetOptsSig(int argc, char **argv, options* opts_,int id) {
	...
	static struct option longopts[] = { { "help", no_argument, NULL, 'h' }, //1
			...
			{ "initial", required_argument, NULL, 'n' }, //12
			{ "geometry", required_argument, NULL, 'g' }, //9
			{ "rotate", required_argument, NULL, 'r' }, //new
			{ "mode", required_argument, NULL, 'm' }, { NULL, 0, NULL, 0 } };
	//if((argc != 18 && argc != 23) && argc >= 3 || (argc == 2 && argv[1][0] != '-' && argv[1][1] != 'h') || argc == 1) {
	...
	int ch;
	string tmp;
	while ((ch = getopt_long(argc, argv, "ht:g:i:o:n:G:m:r:", longopts, NULL))
			!= -1) {
		switch (ch) {
		...
		case 't': {
			...
			iss >> opts_->tiltangle;
			if (iss.fail()) {
				if(PRINTERROR)EX_TRACE("Invalid argument '%s'.", optarg);
				return -1;
			}
			if(access(opts_->tiltangle.c_str(),F_OK)!=0)
			{
				if(PRINTERROR)printf("The tilt file does not exist\n");
				return -1;
			}
		}
			break;
		...
		case 'i': {
			...
			if(access(opts_->input.c_str(),F_OK)!=0)
			{
				if(PRINTERROR)printf("The input file does not exist\n");
				return -1;
			}
		}
			break;
		case 'r': {
			...
			getline(iss, tmp, ',');
			opts_->rotate = atof(tmp.c_str());
			if (iss.fail() || (opts_->rotate  != 0 && opts_->rotate  != 1)) {
				if (PRINTERROR)
					EX_TRACE("Invalid argument '%s'.", optarg);
				return -1;
			}
...
//		    printf("haha\n");
			...
			//iss >> temp;
			getline(iss, tmp, ',');
			opts_->gpu_used = atof(tmp.c_str());
			  if (iss.fail()||(opts_->gpu_used!=0&&opts_->gpu_used!=1)) {
		          if(PRINTERROR)EX_TRACE("Invalid argument '%s'.", optarg);
				  return -1;
             }
