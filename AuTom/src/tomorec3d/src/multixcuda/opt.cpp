...
void usageDual()
{
    ...
    printf ("--mpi/-M number\n");
    printf ("  Run the reconstruction with the number of threads on the cluster\n");
    ...
    printf ("  Iteration numbers and relax parameter for the reconstructed method SIRT\n");

    printf ("--mode/-m ASART Itertaion numbers, relax parameter\n");
    printf ("  Iteration numbers and relax parameter for the reconstructed method ASART\n");
...
int GetOptsSig(int argc, char **argv, options* opts_) {
	...
	string tmp;
	while ((ch = getopt_long(argc, argv, "hG:M:t:g:i:o:n:m:", longopts,
			NULL)) != -1) {
		switch (ch) {
