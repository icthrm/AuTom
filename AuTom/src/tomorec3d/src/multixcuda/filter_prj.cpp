...
void write_MRC_image_from_CvMat(CvMat *Mat, char *filepath, int angle) {
	...
	MPI_File fout;
	MPI_File_open(MPI_COMM_WORLD, filepath, MPI_MODE_CREATE | MPI_MODE_RDWR,
			MPI_INFO_NULL, &fout);
	...
	if (angle == 0)
		PRINT_DEBUG_MSG("Mat->cols is %d, Mat->rows is %d\n", Mat->cols, Mat->rows);
...
int SheppLogan(int length, float *shepp) {
	if (length % 2 != 1) {
		PRINT_DEBUG_MSG("length %d is not Odd\n", length);
		exit(1);
	}
...
/*****************************************************************************************************/
int filter_prj_sym(float *prj_real, const char *filter, int filtlength,
		Projection prj, int row_pad, int col_pad, int symmetrize_2D_flag,
		 int id) {
	int i, j;
	...
	//shuangzhou qu diao angle
/*
	if (id == 0 && angle == 0)
		printf(
				"n_x_pad is %d, n_y_pad is %d, n_x_pad_conv is %d, n_x_pad_conv_opt is %d, n_y_pad_opt is %d\n",
...
*/
	...
	IplImage *projection_org = cvCreateImageHeader(cvSize(prj.X, prj.Y),
			IPL_DEPTH_32F, 1);
...
// printf("projection_org is done\n");
	...
	CvRect projection_pad_center_ROI = cvRect(row_pad / 2, col_pad / 2, prj.X,
			prj.Y);
...
// filter_rs_cv (1*251)
	CvMat *filter_rs_cv = create_CvMat_32F_from_float_data(1, filtlength,
			filter_rs);
...
// filter_fs_cv (n_y_pad_opt*n_x_pad_cov_opt)
	...
	cvGetSubRect(projection_conv, &matrix_src,
			cvRect(filtlength / 2, 0, projection_pad->width,
	...
	for (j = 0; j < projection_pad_center_ROI.height; j++) {
		p_image =
				(float *) (projection_pad->imageData
						+ projection_pad_center_ROI.x * sizeof(float)
						+ (projection_pad_center_ROI.y + j)
								* projection_pad->widthStep);
...
/*****************************************************************************************************/
int filter_prj_X_sym(float *prj_real, char *filter, int filtlength,
		Projection prj, int row_pad, int col_pad, int symmetrize_2D_flag,
		int angle, int id) {
	int i, j;
