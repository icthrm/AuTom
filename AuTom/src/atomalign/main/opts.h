...
inline int GetOpts(int argc, char **argv, opts* opts_){
    ...
    if((argc != 9 && argc != 11) && argc >= 3 || (argc == 2 && argv[1][0] != '-' && argv[1][1] != 'h') || argc == 1){
	EX_TRACE("[-i INPUT FILENAME][-a INIT ANGLE FILENAME][-o OUTPUT FILENAME][-n NEW ANGLE FILENAME]([-r IMAGE ROTATION])\n");
	return -1;
    }
    
    int ch;
    while ((ch = getopt_long(argc, argv, "ho:i:a:n:r:", longopts, NULL))!= -1) {
        switch (ch) {
